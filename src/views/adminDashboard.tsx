...
export default function AdminDashboard(props: Props) {
  ...
  async function filterUsers() {
      ...
      setUserPage(1);
      setUserList(await pizzaService.getUsers(1, 10, `*${filterValue}*`));
  }

  async function deleteUser(user: User) {
      await pizzaService.deleteUser(Number(user.id));
      setUserList(await pizzaService.getUsers(userPage, 10, '*'));
    }
                                  ...
                                  <table className="min-w-full divide-y divide-gray-200">
                                      ...
                                      {userList.users.map((user) => (
                                          <tr key={user.id} className="bg-white">
                                              ...
                                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{user.roles?.map((r) => r.role).join(', ') || ''}</td>
                                      ...
                                      <tr>
                                          <td className="px-1 py-1">
                                              <input type="text" ref={filterUserRef} name="filterUser" placeholder="Filter users by name" className="px-2 py-1 text-sm border border-gray-300 rounded-lg" />
